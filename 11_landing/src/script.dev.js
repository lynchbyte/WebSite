...
gui.add(params, 'bloomStrength', 0.0, 50.0).onChange(function (value) {
...
//gui.addColor(debugObject, 'starCol').onChange(() => { starInsMesh.material.color.set(debugObject.starCol) })
...
function makeShaderPlane() {
    ...
    // gui.add(shaderMaterial.uniforms.baseSpeed, 'value').min(-0.5).max(0.5).step(0.001).name('baseSpeed');
    // gui.add(shaderMaterial.uniforms.noiseScale, 'value').min(-10).max(10).step(0.01).name('noiseScale');
